...
{
    public class clsFilegosu : IOSPCrawlerEdge
    {
        ...
        public async Task<bool> setLogin(Microsoft.Web.WebView2.WinForms.WebView2 web, string strID, string strPwd)
        {
            ...
            string strClickStr = "login_form_check();";
            clsUtil.Delay(500);
        ...
        public async Task<bool> isLogin(Microsoft.Web.WebView2.WinForms.WebView2 web)
        {
            ...
            if (strResult.IndexOf("kakao") != -1) //로그인이 했을 경우에는 존재하지 않는 html코드를 찾아서 ""에 집어넣으면 됨..
                return false;
            else
                return true;
        }
        ...
        public bool getPopupInfo(string strHtml, string strURL, ref BOARD_INFO info, List<string> listPopup)
        {
            ...
            HtmlAgilityPack.HtmlNode moneyNode = parser.getNode("dd", "class", "txt focus");
            ...
            HtmlAgilityPack.HtmlNode titleNode = parser.getNode("div", "class", "d_title");
            strTitle = titleNode.InnerText.Trim();

            HtmlAgilityPack.HtmlNode nameNode = parser.getNode("dd", "class", "txt focus s_user");
            strName = nameNode.InnerText.Trim();
            strName = clsUtil.SubStringEx(strName, "", 1, "판매");
        ...
        public bool Parse(string strHtml, int nPageIndex, ref DataTable dtSearchData, List<string> listPopup, string strURL)
        {
            ...
            parser.getInnerTextList2("td", "class", new string[] { "a_title" }, ref listTitle);

            List<string> listSize = new List<string>();
            parser.getInnerTextList2("td", "class", new string[] { "a_size" }, ref listSize);

            List<string> listName = new List<string>();
            parser.getInnerTextList2("td", "class", new string[] { "a_seller" }, ref listName);


            List<string> listCate = new List<string>();
            parser.getInnerTextList2("td", "class", new string[] { "a_class" }, ref listCate);
            ...
            for (int i = 0, j = 0; i < listName.Count; i += 1, j++)
            {
                ...
                string strSubURL = "https://filegosu.com/" + listNumber[i];
